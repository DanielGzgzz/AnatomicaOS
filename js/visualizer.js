...
Object.assign(window.app, {
    initWebGL() {
        ...
        this.MovementMap = {
            ...
            "hammer_curl": { primary: ['bicep_l', 'bicep_r', 'forearm_l', 'forearm_r'], synergists: ['delt_l', 'delt_r'] },
    ...
    updateProceduralMotion(delta, breath) {
        ...
        let t = {
            ...
            t.groupPosY = -2.0;
            t.groupPosZ = 0;
            t.shoulderFlexion = 1.0 - (phase * 0.5);
            ...
            t.torsoHinge = phase * 1.0;
            t.pelvisZ = phase * -1.5;
            t.hipFlexionL = t.hipFlexionR = phase * 1.6;
            t.kneeFlexionL = t.kneeFlexionR = phase * 0.5;
            t.shoulderFlexion = -t.torsoHinge + 0.2;
            t.elbowFlexion = 0;
        } else if (this.animState === 'pullup') {
            phase = (Math.sin(this.time * 2.5) + 1) / 2;
            t.groupPosY = 3.0 + phase * 3.0;
            t.groupPosZ = -1.0;
            t.shoulderFlexion = -2.5 + (phase * 1.0);
            ...
            t.kneeFlexionL = phase * 1.5;
            t.hipFlexionR = phase * -0.2;
            t.kneeFlexionR = phase * 1.8;
        } else if (this.animState === 'plank') {
            t.groupRotX = Math.PI / 2.2;
            t.groupPosY = -7.2;
            t.groupPosZ = -4.0;
            t.shoulderFlexion = -Math.PI / 2.1;
            t.elbowFlexion = 1.5;
            ...
            t.elbowFlexion = phase * 2.2;
        } else if (this.animState === 'curl' || this.animState === 'hammer_curl') {
            phase = (Math.sin(this.time * 3) + 1) / 2;
            t.shoulderFlexion = 0.1;
            t.elbowFlexion = phase * 2.0;
        ...
        if (['idle', 'plank'].includes(this.animState)) {
             ...
             if (['squat', 'deadlift', 'bench', 'press', 'row', 'overhead_press', 'fullbody'].includes(this.animState)) {
                  ...
                  if (this.dbL) this.dbL.visible = false;
                  if (this.dbR) this.dbR.visible = false;
             } else if (['curl', 'hammer_curl', 'lunge'].includes(this.animState)) {
                  if (this.dbL) {
                      this.dbL.visible = true;
                      this.bodyParts['hand_l'].mesh.getWorldPosition(this.dbL.position);
                      if (this.animState === 'curl') this.dbL.rotation.z = Math.PI / 2;
                      else if (this.animState === 'hammer_curl') this.dbL.rotation.z = 0;
                      else this.dbL.rotation.z = Math.PI / 2;
                  }
                  if (this.dbR) {
                      this.dbR.visible = true;
                      this.bodyParts['hand_r'].mesh.getWorldPosition(this.dbR.position);
                      if (this.animState === 'curl') this.dbR.rotation.z = Math.PI / 2;
                      else if (this.animState === 'hammer_curl') this.dbR.rotation.z = 0;
                      else this.dbR.rotation.z = Math.PI / 2;
                  }
                  if (this.barbell) this.barbell.visible = false;
                  if (this.pullupBar) this.pullupBar.visible = false;
             } else if (this.animState === 'pullup') {
    ...
    playDictionaryExercise() {
        ...
        const vDet = document.getElementById('vis-academic-data');
        ...
        if (exType === 'squat') {
            desc = "Bilateral multi-joint hinge prioritizing knee and hip extension.";
            stds = "Elite: 2.0x Bodyweight\nAdv: 1.5x Bodyweight\nInt: 1.2x Bodyweight";
        } else if (exType === 'bench' || exType === 'press') {
            desc = "Horizontal pressing motion loading the anterior thorax and triceps brachii.";
            stds = "Elite: 1.5x Bodyweight\nAdv: 1.2x Bodyweight\nInt: 1.0x Bodyweight";
        } else if (exType === 'deadlift') {
            desc = "Posterior chain hinge demanding high isometric erector spinae contraction.";
            stds = "Elite: 2.5x Bodyweight\nAdv: 2.0x Bodyweight\nInt: 1.5x Bodyweight";
        } else if (exType === 'pullup') {
            desc = "Vertical pull isolating latissimus dorsi and scapular retractors.";
            stds = "Elite: 15+ Reps\nAdv: 10-14 Reps\nInt: 5-9 Reps";
        } else if (exType === 'plank') {
            desc = "Isometric anti-extension core stabilization.";
            stds = "90th Pctl: >3:00 min\n50th Pctl: 1:30 min\n20th Pctl: 0:45 min";
        } else if (exType === 'lunge') {
            desc = "Unilateral closed-chain leg extension.";
            stds = "Adv: 0.75x BW per hand\nInt: 0.5x BW per hand";
        } else if (exType === 'overhead_press') {
            desc = "Vertical pressing loading anterior/medial deltoids.";
            stds = "Elite: 1.0x Bodyweight\nAdv: 0.8x Bodyweight\nInt: 0.6x Bodyweight";
        } else if (exType === 'row') {
            desc = "Horizontal pull targeting rhomboids and latissimus dorsi thickness.";
            stds = "Elite: 1.5x Bodyweight\nAdv: 1.2x Bodyweight\nInt: 0.8x Bodyweight";
        } else if (exType === 'curl' || exType === 'hammer_curl') {
            desc = exType === 'hammer_curl' ? "Neutral grip flexion targeting brachioradialis." : "Supinated elbow flexion isolating biceps brachii.";
            stds = "Rep Range: 8-15\nHypertrophy Focus";
        } else if (exType === 'crunch') {
            desc = "Spinal flexion isolating rectus abdominis.";
            stds = "Endurance: 50+ Reps";
        } else if (exType === 'calf_raise') {
            desc = "Plantar flexion isolating gastrocnemius.";
            stds = "Hypertrophy: 15-20 Reps";
        } else if (exType === 'leg_extension') {
            desc = "Open-chain knee extension isolating quadriceps.";
            stds = "Hypertrophy: 10-15 Reps";
        } else {
            desc = "Isolated biomechanical movement.";
            stds = "Load dynamically based on CNS fatigue.";
        }

        const dDesc2 = document.getElementById('acad-desc');
        if (dDesc2) dDesc2.innerText = desc;
        const dEq2 = document.getElementById('acad-standards');
        if (dEq2) dEq2.innerText = stds;
    },
    updateWebGLVisualizer() {
        ...
        let desc = "ISOLATING: Rest | Protocol: Recovery";
        let stds = "Protocol: Static stretching, parasympathetic breathing.";

        if (day.focus.includes('Lower Body')) { this.animState = 'squat'; desc = "Bilateral multi-joint hinge prioritizing knee and hip extension."; stds = "Elite: 2.0x Bodyweight"; }
        else if (day.focus.includes('Upper Body')) { this.animState = 'press'; desc = "Horizontal pressing motion loading the anterior thorax and triceps brachii."; stds = "Elite: 1.5x Bodyweight"; }
        else if (day.focus.includes('Full Body Strength')) { this.animState = 'fullbody'; desc = "Full chain integration targeting maximum motor unit recruitment."; stds = "1RM - 3RM Intensity"; }
        else if (day.focus.includes('Maximal Output')) { this.animState = 'deadlift'; desc = "Posterior chain hinge demanding high isometric erector spinae contraction."; stds = "Elite: 2.5x Bodyweight"; }
        else if (day.focus.includes('Core')) { this.animState = 'plank'; desc = "Isometric anti-extension core stabilization."; stds = "90th Pctl: >3:00 min"; }
